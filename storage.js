...
export function read(){const raw=localStorage.getItem(KEY);if(!raw)return fresh();const old=validate(JSON.parse(raw));if(old.schemaVersion===3)return old;const next=validate(migrate(old));localStorage.setItem(KEY+'-before-v3',raw);write(next);return next;}
